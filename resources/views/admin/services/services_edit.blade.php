					...
					<div class="card-body min_height">
					    <form class="row" id="Services_add_form" enctype="multipart/form-data"  action="javascript:void(0)" method="post">
							...
							<div class="mb-3 col-md-6">
								<label for="recipient-name" class="control-label">Select Services Category:<span style="color:red">*</span></label>
								<select class="form-control" required id="Services_category" name ="Services_category_id">
                                <option value="">Select Services Category</option>    
                                @if(!empty($Services_categories))
                                        @foreach($Services_categories as $Servicescat)
                                            <option value="{{$Servicescat->id}}">{{$Servicescat->name}}</option>
                                        @endforeach
                                     @endif
								</select>
							</div>
							<div class="mb-3 col-md-6">
								<label for="recipient-name" class="control-label">Services Title:<span style="color:red">*</span></label>
								<input type="text" class="form-control" value="" id="Services_title" name="Services_title" required>
							</div>
							<div class="mb-3 col-md-6">
							...
							<div class="mb-3 col-md-6">
								<label for="recipient-name" class="control-label">Services Publish Date:<span style="color:red">*</span></label>
								<input type="date" class="form-control" value="" id ="Services_public_date" required name="Services_public_date">
							</div>

                            <div class="mb-3 col-md-6">
								<label for="recipient-name" class="control-label">Services Description:<span style="color:red">*</span></label>
								<textarea cols="80" id="Services_description" name="Services_description" rows="10" required data-sample="2" class="form-control"></textarea>

                            </div>
							<div class="mb-3 col-md-6">
								<div class="mb-3 col-md-6">
									<label for="recipient-name" class="control-label">Services Image <small class="text-danger"><span style="color:red">*</span></small></label>
									<input class="form-control" type="file" id="formFile" name ="Servicesimage" required>
...
<script>
    ...
    $('#Services_public_date').attr('min', minDate);
});
